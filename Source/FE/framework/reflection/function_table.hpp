...
public:
	...
	static void register_method(underlying_container::value_type value_p) noexcept { tl_s_hash_map.insert(value_p); }
	static var::boolean check_presence(underlying_container::key_type key_p) noexcept { return (tl_s_hash_map.find(key_p) == tl_s_hash_map.end()) ? false : true; }
	static FE::task_base* retrieve(underlying_container::key_type key_p) noexcept { return tl_s_hash_map.find(key_p)->second; }

	template<typename R>
	_FORCE_INLINE_ static R invoke(underlying_container::key_type key_p) noexcept
	{
		R l_buffer;
		tl_s_hash_map.find(key_p)->second->operator()(&l_buffer);
		return std::move(l_buffer);
	}
	_FORCE_INLINE_ static void invoke(underlying_container::key_type key_p) noexcept
	{
		tl_s_hash_map.find(key_p)->second->operator()(nullptr);
	}
};
