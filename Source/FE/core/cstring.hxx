...
public:
    ...
    {
        return (algorithm::string::search_first_substring(this->data(), substring_ref_p.data())).has_value();
    }
    _FORCE_INLINE_ constexpr var::boolean contains(const char_type target_character_p) const noexcept
    {
        algorithm::string::char_search_result<char_type> l_target_char_search_result = algorithm::string::search_first_char(this->data(), target_character_p);
    ...
    _FORCE_INLINE_ constexpr FE::optional<algorithm::string::range> find(compile_time_constant_string<char_type>& target_substring_ref_p, index_t position_p = 0) const noexcept
    {
        ...
        FE::optional<algorithm::string::range> l_result = algorithm::string::search_first_substring(this->data() + position_p, target_substring_ref_p.data());
    ...
    _FORCE_INLINE_ constexpr FE::optional<algorithm::string::range> find(const char_type* const target_substring_ptrc_p, index_t position_p = 0) const noexcept
    {
        FE_ASSERT(target_substring_ptrc_p == nullptr, "${%s@0}: target_substring_ptrc_p is nullptr", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
        FE_ASSERT(position_p > algorithm::string::string_length(this->data()), "ERROR: position_p cannot be greater than the left operand string length.");
        FE::optional<algorithm::string::range> l_result = algorithm::string::search_first_substring(this->data() + position_p, target_substring_ptrc_p);
        l_result->_begin += position_p;
        l_result->_end += position_p;
        return l_result;
    }
    ...
    {
        FE_ASSERT(position_p > algorithm::string::string_length(this->data()), "ERROR: position_p cannot be greater than the left operand string length.");
        algorithm::string::char_search_result l_result = algorithm::string::search_first_char(this->data() + position_p, target_character_p);
        l_result->_begin += position_p;
        l_result->_end += position_p;
        return l_result;
    }
