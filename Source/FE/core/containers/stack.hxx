...
public:
	...
	_CONSTEXPR20_ _FORCE_INLINE_ fstack(std::initializer_list<value_type>&& initializer_list_p) noexcept : m_memory(), m_top_ptr(reinterpret_cast<pointer>(m_memory)), m_absolute_begin_ptrc(m_top_ptr)
	{
	...
	_CONSTEXPR20_ _FORCE_INLINE_ fstack(fstack& other_ref_p) noexcept : m_memory(), m_top_ptr(reinterpret_cast<pointer>(m_memory)), m_absolute_begin_ptrc(m_top_ptr)
	{
	...
	_CONSTEXPR20_ _FORCE_INLINE_ fstack(fstack&& rvalue_p) noexcept : m_memory(), m_top_ptr(reinterpret_cast<pointer>(m_memory)), m_absolute_begin_ptrc(m_top_ptr)
	{
	...
	_CONSTEXPR20_ _FORCE_INLINE_ fstack& operator=(std::initializer_list<value_type> initializer_list_p) noexcept
	{
		FE_ASSERT(initializer_list_p.size() > max_element_count, "ERROR!: The length of std::initializer_list exceeds the max_element_count");
		FE_ASSERT(initializer_list_p.size() == 0, "${%s@0}!: Cannot assign an empty initializer_list", TO_STRING(MEMORY_ERROR_1XX::_ERROR_INVALID_SIZE));
	...
	_CONSTEXPR20_ _FORCE_INLINE_ fstack& operator=(fstack& other_ref_p) noexcept
	{
	...
	_CONSTEXPR20_ _FORCE_INLINE_ fstack& operator=(fstack&& rvalue_p) noexcept
	{
	...
	_CONSTEXPR20_ _FORCE_INLINE_ const_reference top() const noexcept
	{
		return *(this->m_top_ptr - 1);
	}

	_NODISCARD_ _CONSTEXPR20_ _FORCE_INLINE_ var::boolean is_empty() noexcept
	{
	...
	_CONSTEXPR20_ _FORCE_INLINE_ void swap(fstack& other_ref_p) noexcept
	{
	...
	_CONSTEXPR20_ _NODISCARD_ _FORCE_INLINE_ boolean operator==(fstack& other_ref_p) noexcept
	{
...
private:
	_CONSTEXPR20_ _FORCE_INLINE_ void __jump_top_pointer(difference_type ptrdiff_p) noexcept
	{
		this->m_top_ptr += ptrdiff_p;
	}

	_CONSTEXPR20_ _FORCE_INLINE_ void __set_top_pointer_to_zero() noexcept
	{
		this->m_top_ptr = this->m_absolute_begin_ptrc;
	}

	_CONSTEXPR20_ _FORCE_INLINE_ void __restructrue_fstack_with_move_semantics(value_type* const source_begin_ptrc_p, size_t source_size_p) noexcept
	{
	...
	_CONSTEXPR20_ _FORCE_INLINE_ void __restructrue_fstack_with_copy_semantics(value_type* const source_begin_ptrc_p, size_t source_size_p) noexcept
	{
		size_t l_this_size = this->size();
