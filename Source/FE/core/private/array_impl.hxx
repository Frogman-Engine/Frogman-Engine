...
public:
	...
	{
		FE_STATIC_ASSERT(((std::is_class<InputIterator>::value == false) && (std::is_pointer<InputIterator>::value == false)), "Static Assertion Failure: The template argument InputIterator must be a class");
...
<<<<<<< HEAD
	...
	_FORCE_INLINE_ reference operator[](const size_type index_p) noexcept
=======
...
<<<<<<< HEAD
	_FORCE_INLINE_ const_reference operator[](const size_type index_p) const noexcept
	{
		FE_ASSERT(this->m_smart_array.get() == nullptr, "Unable to access an uninitialized container.");
		FE_ASSERT(index_p >= this->m_array_size, "Assertion Failure: ${%s@0} must not be greater than ${%s@1}.", TO_STRING(index_p), TO_STRING(this->m_array_size));
	...
	_FORCE_INLINE_ reference front() noexcept
=======
...
<<<<<<< HEAD
	_FORCE_INLINE_ const_pointer data() const noexcept
	{
		FE_ASSERT(this->m_smart_array.get() == nullptr, "Unable to access an uninitialized container.");
		return this->m_smart_array.get();
	}

	_FORCE_INLINE_ pointer data() noexcept
=======
