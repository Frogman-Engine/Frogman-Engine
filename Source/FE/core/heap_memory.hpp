...
template<typename T, class AddressAlignment>
...
#ifdef _AVX512_
template<typename U, class AddressAlignment = align_64bytes>
#elif defined(_AVX_)
template<typename U, class AddressAlignment = align_32bytes>
#endif
class heap_memory_tracker final : public heap_memory_tracker_base
{
	friend class scalable_aligned_allocator<U, AddressAlignment>;
...
public:
	...
	_FORCE_INLINE_ static void __log_heap_memory_reallocation(uint64 prev_size_in_bytes_p, uint64 new_size_in_bytes_to_allocate_p, const char* const allocator_name_p, U* const allocated_address_p, const char* const allocated_type_name_p) noexcept
	{
		...
		FE_LOG("\n\n The previous total heap memory used by this application was ${%lu@0} bytes. \n The previous total heap memory used by this thread[id:${%lu@1}] was ${%lu@2} bytes.\n Re-allocated ${%lu@3} bytes of heap memory from ${%s@4}. \n The memory address is aligned by ${%d@5} bytes. \n The allocated memory data type is ${%s@6}.\n The allocated memory address is 0x${%p@7} \n", &(l_prev_heap_mem_usage_in_bytes._global_total_bytes), &l_this_thread_id, &(l_prev_heap_mem_usage_in_bytes._thread_local_total_bytes), &new_size_in_bytes_to_allocate_p, allocator_name_p, &AddressAlignment::size, allocated_type_name_p, allocated_address_p);
	...
	_FORCE_INLINE_ static void __log_heap_memory_deallocation(uint64 size_in_bytes_to_deallocate_p, const char* const allocator_name_p, U* const allocated_address_p, const char* const allocated_type_name_p) noexcept
	{
		...
		FE_LOG("\n\n The previous total heap memory used by this application was ${%lu@0} bytes. \n The previous total heap memory used by this thread[id:${%lu@1}] was ${%lu@2} bytes.\n De-allocated ${%lu@3} bytes of heap memory via ${%s@4}. \n The memory address is aligned by ${%d@5} bytes. \n The de-allocated memory data type is ${%s@6}.\n The de-allocated memory address is 0x${%p@7} \n", &(l_prev_heap_mem_usage_in_bytes._global_total_bytes), &l_this_thread_id, &(l_prev_heap_mem_usage_in_bytes._thread_local_total_bytes), &size_in_bytes_to_deallocate_p, allocator_name_p, &AddressAlignment::size, allocated_type_name_p, allocated_address_p);
	...
	template<typename T, class Alignment>
	friend _FORCE_INLINE_ void trackable_free(T* const memblock_pointer_p, length_t count_p, size_t bytes_p) noexcept;

	template<typename T, class Alignment>
	friend _FORCE_INLINE_ T* trackable_realloc(T* const memblock_pointer_p, length_t prev_length_p, size_t prev_bytes_p, length_t new_length_p, size_t new_bytes_p) noexcept;
};


template<typename T, class AddressAlignment>
::std::atomic_int64_t FE::heap_memory_tracker<T, AddressAlignment>::s_global_total_bytes_by_type = 0;

template<typename T, class AddressAlignment>
thread_local var::int64 FE::heap_memory_tracker<T, AddressAlignment>::tl_s_thread_local_total_bytes_by_type = 0;
...
template<typename T, class Alignment = align_32bytes>
#endif
_FORCE_INLINE_ void trackable_free(T* const memblock_pointer_p, _MAYBE_UNUSED_ length_t count_p, _MAYBE_UNUSED_ size_t bytes_p) noexcept
{
	::scalable_aligned_free(memblock_pointer_p);

#if defined(_ENABLE_MEMORY_TRACKER_)
	::FE::heap_memory_tracker<T, Alignment>::__log_heap_memory_deallocation(count_p * bytes_p, TO_STRING(trackable_free), memblock_pointer_p, typeid(T).name());
#endif 
}
...
template<typename T, class Alignment = align_32bytes>
#endif
_NODISCARD_ _FORCE_INLINE_ T* trackable_realloc(T* const memblock_pointer_p, length_t prev_length_p, size_t prev_bytes_p, length_t new_length_p, size_t new_bytes_p) noexcept
{
	T* l_realloc_result_ptr = (T*)::scalable_aligned_realloc(memblock_pointer_p, new_bytes_p * new_length_p, Alignment::size);
	...
	if (l_realloc_result_ptr == nullptr) _UNLIKELY_
	{
		...
		FE::aligned_memcpy(l_realloc_result_ptr, new_length_p, new_bytes_p, memblock_pointer_p, prev_length_p, prev_bytes_p);
		
		if (l_realloc_result_ptr != memblock_pointer_p) _LIKELY_
		{
			::scalable_aligned_free(memblock_pointer_p);
		}
	}
