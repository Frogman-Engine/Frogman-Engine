...
public:
    ...
    _CONSTEXPR20_ fixed_sized_string(InputIterator first_p, InputIterator last_p) noexcept : m_length(static_cast<length_t>(last_p - first_p))
    {
        ...
        FE_ASSERT(static_cast<uint64>(last_p - first_p) > Capacity, "${%s@0}: The input size exceeds the fixed_sized_string capacity.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY));
    ...
    {
        FE_ASSERT(index_p >= this->m_length, "${%s@0}: index out of boundary.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE));
    ...
    {
        FE_ASSERT(count_p >= Capacity, "${%s@0}: ${%s@1} exceeds fixed sized string capacity.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY), TO_STRING(count_p));
    ...
    _CONSTEXPR20_ fixed_sized_string& assign(const_iterator input_begin_p, const_iterator input_end_p) noexcept
    {
        ...
        FE_ASSERT(static_cast<uint64>(input_end_p - input_begin_p) >= Capacity, "${%s@0}: input string range length exceeds fixed sized string capacity.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE));
    ...
    {
        FE_ASSERT(position_p >= Capacity, "${%s@0}: position_p cannot be greater than the string capacity.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE));
    ...
    {
        FE_ASSERT(position_p >= Capacity, "${%s@0}: ${%s@1} cannot be greater than the ${%s@2}.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY), TO_STRING(position_p), TO_STRING(Capacity));
        FE_ASSERT((position_p + other_p.m_length) >= Capacity, "${%s@0}: fixed sized string capacity overflowed.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY));
    ...
    _CONSTEXPR20_ fixed_sized_string& insert(const size_type position_p, std::initializer_list<const CharT>&& initializer_list_p) noexcept
    {
        ...
        FE_ASSERT((position_p + initializer_list_p.size()) >= Capacity, "${%s@0}: fixed sized string capacity overflowed.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY));
    ...
    {
        FE_ASSERT(Capacity < (index_p + count_p), "${%s@0}: fixed sized string capacity overflowed.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE));
    ...
    _CONSTEXPR20_ fixed_sized_string& append(const size_type input_count_p, const value_type value_p) noexcept
    {
        ...
        FE_ASSERT(this->max_length() < (this->m_length + input_count_p), "${%s0}: cannot append ${%ld@1} character(s) to the fstring.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE), &input_count_p);
    ...
    {
        FE_ASSERT(initializer_list_p.size() == 0, "${%s@0}: The initializer_list_p is empty.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_INVALID_SIZE));
        FE_ASSERT(this->max_length() < initializer_list_p.size() + this->m_length, "${%s@0}: initializer_list_p is too large to take the contents.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY));
    ...
    _NODISCARD_ _CONSTEXPR20_ boolean starts_with(const fixed_sized_string& other_p) const noexcept
    {
        ...
        return algorithm::string::compare_ranged<CharT>(this->m_fstring, algorithm::string::range{0, other_p.m_length},
    ...
    {
        return algorithm::string::compare_ranged<CharT>(this->m_fstring, algorithm::string::range{0, 1},
    ...
    _NODISCARD_ _CONSTEXPR20_ boolean starts_with(const value_type* const string_p) const noexcept
    {
        ...
        return algorithm::string::compare_ranged<CharT>(this->m_fstring, algorithm::string::range{0, l_substring_length},
    ...
    _NODISCARD_ _CONSTEXPR20_ boolean ends_with(const fixed_sized_string& other_p) const noexcept
    {
        ...
        return algorithm::string::compare_ranged<CharT>(this->m_fstring, algorithm::string::range{this->m_length - other_p.m_length, this->m_length},
            other_p.data(), algorithm::string::range{ 0, other_p.m_length }
        );
    }
    ...
    {
        return algorithm::string::compare_ranged<CharT>(this->m_fstring, algorithm::string::range{this->m_length - 1, this->m_length},
            & value_p, algorithm::string::range{ 0, 1 }
        );
    }
    ...
    _NODISCARD_ _CONSTEXPR20_ boolean ends_with(const value_type* const string_p) const noexcept
    {
        FE_ASSERT(string_p == nullptr, "${%s@0}: ${%s@1} is nullptr", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_NULLPTR), TO_STRING(string_p));
        length_t l_substring_length = algorithm::string::length(string_p);
        return algorithm::string::compare_ranged<CharT>(this->m_fstring, algorithm::string::range{this->m_length - l_substring_length, this->m_length},
            string_p, algorithm::string::range{ 0, l_substring_length }
        );
    }
    ...
    {
        FE_ASSERT(((this->m_length + other_p.m_length) - count_to_be_removed_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY));
    ...
    {
        FE_ASSERT(position_p + count_to_be_removed_p > this->m_length, "${%s@0}: failed to replace().", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE));
        FE_ASSERT(other_position_p + other_count_p > other_p.m_length, "${%s@0}: failed to replace().", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE));
        FE_ASSERT(count_to_be_removed_p == 0, "${%s@0}: ${%s@1} is zero.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_INVALID_SIZE), TO_STRING(count_to_be_removed_p));
    ...
    _CONSTEXPR20_ fixed_sized_string& replace(const size_type position_p, const size_type count_to_be_removed_p, const value_type* const string_p, const size_type input_count_p) noexcept
    {
        ...
        FE_ASSERT(((this->m_length + input_count_p) - count_to_be_removed_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY));
        FE_ASSERT(count_to_be_removed_p == 0, "${%s@0}: ${%s@1} is zero.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_INVALID_SIZE), TO_STRING(count_to_be_removed_p));
        FE_ASSERT(string_p == nullptr, "${%s@0}: ${%s@1} is nullptr.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_NULLPTR), TO_STRING(string_p));
    ...
    _CONSTEXPR20_ fixed_sized_string& replace(const size_type position_p, const size_type count_to_be_removed_p, const value_type* const string_p) noexcept
    {
        ...
        FE_ASSERT(((this->m_length + l_input_length) - count_to_be_removed_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY));
    ...
    _CONSTEXPR20_ fixed_sized_string& replace(const size_type position_p, const size_type count_to_be_removed_p, std::initializer_list<const CharT>&& initializer_list_p) noexcept
    {
        ...
        FE_ASSERT(((this->m_length + l_input_size) - count_to_be_removed_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_CAPACITY));
        FE_ASSERT(count_to_be_removed_p == 0, "${%s@0}: ${%s@1} is zero.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_INVALID_SIZE), TO_STRING(count_to_be_removed_p));
    ...
    _CONSTEXPR20_ fixed_sized_string& replace_with_range(const size_type position_p, const size_type count_to_be_removed_p, const_iterator input_iterator_begin_p, algorithm::string::range input_string_range_p) noexcept
    {
        ...
        FE_ASSERT(position_p > this->max_length(), "${%s@0}: ${%s@1} must not be greater than ${%s@2}.", TO_STRING(ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_INVALID_SIZE), TO_STRING(position_p), TO_STRING(this->max_length()));
    ...
    _NODISCARD_ _CONSTEXPR20_ iterator rfind(const value_type value_p, const size_type position_p = 0) const noexcept
    {
        ...
        std::optional<algorithm::string::range> l_result = algorithm::string::find_the_last_within_range(this->m_fstring, algorithm::string::range{position_p, this->m_length}, value_p);
