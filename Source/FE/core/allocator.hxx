...
public:
	...
	_NODISCARD_ _FORCE_INLINE_ static pointer allocate(size_type count_p) noexcept
	{
		...
		FE_ASSERT(MODULO_BY_64(reinterpret_cast<uintptr_t>(l_result_ptrc)) != 0, "${%s@0}: The allocated heap memory address not aligned by sixty-four. The address value was ${%p@1}", TO_STRING(MEMORY_ERROR_1XX::_ERROR_ILLEGAL_ADDRESS_ALIGNMENT), l_result_ptrc);
