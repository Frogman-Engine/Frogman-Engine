...
protected:
	...
	template<typename T, class alignment = FE::align_null, FE::OBJECT_TRIVIALITY object_triviality = FE::is_trivially_constructible_and_destructible<T>::_VALUE_>
	_FORCE_INLINE_ static void swap(optional<T, object_triviality>& left_p, optional<T, object_triviality>& right_p)
	{
		optional<T, alignment, object_triviality> l_temporary = std::move(left_p);
