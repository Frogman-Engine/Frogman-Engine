...
public:
	...
	_NODISCARD_ _FORCE_INLINE_ static T* allocate(count_t count_p) noexcept
	{
		...
		{
		case 0:
			FE_ASSERT(true, "UNRECOVERABLE CRITICAL ERROR!: queried allocation size is zero.");
			break;
		...
		case 1:
			...
			FE_ASSERT(l_allocation_result_ptr == nullptr, "UNRECOVERABLE CRITICAL ERROR!: failed to allocate memory from new_delete_allocator.");
	...
	{
		FE_ASSERT(new_count_p == 0, "UNRECOVERABLE CRITICAL ERROR!: queried reallocation size is zero.");
	...
	{
		FE_ASSERT(ptrc_p == nullptr, "UNRECOVERABLE CRITICAL ERROR!: attempted to delete nullptr.");

		switch (count_p)
		{
		case 0:
			FE_ASSERT(true, "UNRECOVERABLE CRITICAL ERROR!: attempted to delete zero byte.");
			break;
		case 1:
#if _ENABLE_MEMORY_TRACKER_ == true
