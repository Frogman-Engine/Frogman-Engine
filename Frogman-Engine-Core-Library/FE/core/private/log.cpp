...
logger* logger::s_logging_strategy_ptr = nullptr;

thread_local ::std::ofstream logger::tl_s_file_logger;
thread_local ::FE::clock logger::tl_s_clock;
thread_local ::std::string logger::tl_s_log_buffer;
::FE::lazy_const<FE::var::size_t> logger_initialization_arguments::s_log_buffer_size;
::FE::lazy_const<FE::var::uint8> logger_initialization_arguments::s_write_operation_triggering_point;
...
void real_time_logging_strategy::__logging_strategy(character* const message_ptrc_p, character* const file_name_ptrc_p, character* const function_name_ptrc_p, int32 line_p) noexcept
{
...
void real_time_logging_strategy::__log_initialization_strategy() noexcept
{
    ...
    ::std::mbstowcs(l_date_info_wstring, tl_s_clock.get_current_local_time(), ::FE::clock::_GET_CURRENT_LOCAL_TIME_BUFFER_SIZE_);
...
void real_time_logging_strategy::__log_deinitialization_strategy() noexcept
{
...
void log_buffering_strategy::__logging_strategy(character* const message_ptrc_p, character* const file_name_ptrc_p, character* const function_name_ptrc_p, int32 line_p) noexcept
{
    buffer_type l_source_code_line_info_buffer[_LINE_INFO_BUFFER_SIZE_] = "\0";
...
void log_buffering_strategy::__log_initialization_strategy() noexcept
{
    ::std::filesystem::path l_directory_name = (::std::filesystem::current_path() / "Frogman-Engine-Logs\0");
    if (::std::filesystem::exists(l_directory_name) == false)
    {
...
void log_buffering_strategy::__log_deinitialization_strategy() noexcept
{
    if (tl_s_file_logger.is_open() == true)
    {
        tl_s_file_logger << "-------------------------------------------------- END OF LOG --------------------------------------------------\n";
