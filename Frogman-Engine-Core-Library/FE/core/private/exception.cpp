...
FE::exception* FE::exception::s_exception_handling_strategy_ptr = nullptr;

thread_local ::std::ofstream FE::real_time_exception_history_logging_strategy::tl_s_exception_reporter;
...
{
    exception::s_exception_handling_strategy_ptr->__main_thread_exception_construction_strategy();
}

void  FE::exception::__destruct_exception_on_main_thread() noexcept
{
    exception::s_exception_handling_strategy_ptr->__main_thread_exception_destruction_strategy();
}

void FE::exception::__construct_exception() noexcept
{
    exception::s_exception_handling_strategy_ptr->__exception_construction_strategy();
}

void FE::exception::__destruct_exception() noexcept
{
    exception::s_exception_handling_strategy_ptr->__exception_destruction_strategy();
}




thread_local ::std::unique_ptr<FE::exception::char_type[]> FE::real_time_exception_history_logging_strategy::tl_s_source_code_line_info_buffer IF_DEBUG_MODE((new FE::exception::char_type[_LINE_INFO_BUFFER_SIZE_]{'\0'}), = nullptr);

thread_local ::std::unique_ptr<FE::exception::char_type[]> FE::real_time_exception_history_logging_strategy::tl_s_full_debug_info_buffer IF_DEBUG_MODE((new FE::exception::char_type[_FULL_DEBUG_INFO_BUFFER_SIZE_]{ '\0' }), = nullptr);
...
bool FE::real_time_exception_history_logging_strategy::__exception_handling_strategy(const bool expression_p, const char* const expression_string_ptrc_p, const FE::EXCEPTION_MODE runtime_exception_mode_p, const char* message_ptr_p, const char* file_name_ptr_p, const char* function_name_ptr_p, const int line_p, const int exit_code_p) noexcept
{
    ...
    case _LOG_EXCEPTION_HISTORY_: // store exception logs in a global container and write  exception history file right before the unsafe_end of the application.  
        real_time_exception_history_logging_strategy::tl_s_exception_reporter << real_time_exception_history_logging_strategy::tl_s_full_debug_info_buffer.get() << "\n\n";
...
void FE::real_time_exception_history_logging_strategy::__main_thread_exception_construction_strategy() noexcept
{
    ...
    real_time_exception_history_logging_strategy::tl_s_exception_reporter.open(l_exception_history_file_name.c_str());
    real_time_exception_history_logging_strategy::tl_s_exception_reporter << "[BEGIN RECORD]\n{\n";
...
{
    if (real_time_exception_history_logging_strategy::tl_s_exception_reporter.is_open() == true)
    {
        real_time_exception_history_logging_strategy::tl_s_exception_reporter << "\n}\n[END OF HISTORY]";
        ...
        real_time_exception_history_logging_strategy::tl_s_exception_reporter << "The total leaked byte(s) on the heap is " << heap_utilization<void>::query_all_data()._global_total_bytes << " byte(s)";
        real_time_exception_history_logging_strategy::tl_s_exception_reporter.close();
    }
}
