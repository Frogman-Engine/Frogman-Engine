...
public:
	...
	{
		FE_ASSERT(this->m_begin >= this->m_end, "${%s@0}: Unable to shrink the front of an empty string view", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE));
		this->m_begin += count_p;
	}

	_CONSTEXPR17_ void remove_suffix(const size_type count_p) noexcept
	{
		FE_ASSERT(this->m_begin >= this->m_end, "${%s@0}: Unable to shrink the back of an empty string view", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE));
		this->m_end -= count_p;
	}
	...
	_CONSTEXPR20_ void copy(CharT* const out_out_dest_buffer_pointer_p, const size_type buffer_capacity_p, const size_type count_p, const size_type position_p = 0) const noexcept
	{
		...
		FE_ASSERT(count_p > this->length(), "${%s@0}: ${%s@1} cannot be greater than ${%s@2}.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE), TO_STRING(count_p), TO_STRING(this->length()));
	...
	_NODISCARD_ _CONSTEXPR20_ std::optional<algorithm::string::range> find(const basic_string_view& other_p, size_type position_p = 0) const noexcept
	{
		...
		FE_ASSERT(position_p > this->length(), "${%s@0}: ${%s@1} cannot be greater than ${%s@2}.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE), TO_STRING(position_p), TO_STRING(this->length()));
	...
	_NODISCARD_ _CONSTEXPR20_ std::optional<algorithm::string::range> find(const CharT* const string_p, const size_type position_p = 0) const noexcept
	{
		...
		FE_ASSERT(position_p > this->length(), "${%s@0}: position_p cannot be greater than ${%s@1}.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE), TO_STRING(this->length()));
	...
	{
		FE_ASSERT(position_p > this->length(), "${%s@0}: position_p cannot be greater than ${%s@1}.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE), TO_STRING(this->length()));
		FE_ASSERT(this->m_begin > position_p, "${%s@0}: position_p cannot be less than or equal to ${%s@1}.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_OUT_OF_RANGE), TO_STRING(this->m_begin));
