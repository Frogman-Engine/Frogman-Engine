...
public:
	...
	_FE_FORCE_INLINE_ T* trackable_alloc(FE::size bytes_p) noexcept
	{
		...
		FE_NEGATIVE_ASSERT(l_result == nullptr, "${%s@0}: Failed to allocate memory from scalable_aligned_malloc()", TO_STRING(FE::ErrorCode::_FATAL_MEMORY_ERROR_1XX_NULLPTR));
		FE_NEGATIVE_ASSERT((reinterpret_cast<uintptr>(l_result) % Alignment::size) != 0, "${%s@0}: The allocated heap memory address not aligned by ${%lu@1}.", TO_STRING(FE::ErrorCode::_FATAL_MEMORY_ERROR_1XX_ILLEGAL_ADDRESS_ALIGNMENT), &Alignment::size);
#ifndef _RELEASE_
	...
	{
		FE_NEGATIVE_ASSERT((reinterpret_cast<uintptr>(ptr_to_memory_p) % Alignment::size) != 0, "${%s@0}: The allocated heap memory address not aligned by ${%lu@1}.", TO_STRING(FE::ErrorCode::_FATAL_MEMORY_ERROR_1XX_ILLEGAL_ADDRESS_ALIGNMENT), &Alignment::size);
	...
	_FE_FORCE_INLINE_ T* trackable_realloc(T* const ptr_to_memory_p, size prev_bytes_p, size new_bytes_p) noexcept
	{
		...
		if (l_realloc_result == nullptr) _FE_UNLIKELY_
		{
			...
			FE_NEGATIVE_ASSERT(l_realloc_result == nullptr, "${%s@0}: Failed to re-allocate memory from FE_ALIGNED_ALLOC()", TO_STRING(FE::ErrorCode::_FATAL_MEMORY_ERROR_1XX_NULLPTR));
	#ifndef _RELEASE_
...
#endif
		FE_NEGATIVE_ASSERT((reinterpret_cast<uintptr>(l_realloc_result) % Alignment::size) != 0, "${%s@0}: The allocated heap memory address not aligned by ${%lu@1}.", TO_STRING(FE::ErrorCode::_FATAL_MEMORY_ERROR_1XX_ILLEGAL_ADDRESS_ALIGNMENT), &Alignment::size);
