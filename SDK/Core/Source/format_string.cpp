...
*/
constexpr FE::uint32 __hash_string(const char* string_p, FE::length_t length_p)
{
	FE::uint32 l_number_of_blocks = length_p / 4;
	...
	for (var::int32 i = -l_number_of_blocks; i != 0; ++i) 
	{
...
FormatSpecifier __identify_format_specifier(const char* const string_format_p) noexcept
{
	...
	FE::length_t l_format_specifier_length = l_iterator - string_format_p;
...
void format_string(char* out_buffer_pointer_p, const char* string_format_p, size buffer_size_p, const void** arguments_pointer_p, count_t arguments_count_p) noexcept
{
    ...
    while (*string_format_p != FE::null)
    {
		...
		if (static_cast<FE::count_t>(l_idx) >= arguments_count_p)
		{
