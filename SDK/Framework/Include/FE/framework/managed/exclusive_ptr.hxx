...
public:
	...
	{
		FE_NEGATIVE_ASSERT(this->m_ref_block == nullptr == true, "${%s@0}: The smart pointer was nullptr.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_NULLPTR));
		FE_NEGATIVE_ASSERT(static_cast<index_t>(this->m_ref_block->_end - this->m_ref_block->_address) <= index_p, "${%s@0}: ${%s@1} exceeds the index boundary. ${%s@1} was ${%lu@2}.", TO_STRING(FE::ERROR_CODE::_FATAL_MEMORY_ERROR_1XX_ACCESS_VIOLATION), TO_STRING(index_p), &index_p);
