...
public:
	...
	using upstream_arena_t = FE::scalable_pool<FE::PoolPageCapacity::_1GB>;
	using entity_arena_t = FE::scalable_pool<FE::PoolPageCapacity::_64MB>;
	using component_arena_t = FE::scalable_pool<FE::PoolPageCapacity::_64MB>;
...
public:
    ...
    component_ptr<Component> request_component_attachment(Archetype* const entity_p) noexcept
    {
        ...
        for (auto archetype_it = m_component_table.find(entity_p); archetype_it != m_component_table.end(); ++archetype_it)
        {
			if (archetype_it->first == entity_p)
			{
                std::pmr::string l_identifier(reflection::type_id<Component>().name(), FE::framework::framework_base::get_engine().get_memory_resource());
                auto l_component_it = archetype_it->second.find(l_identifier);

                if (l_component_it == archetype_it->second.end())
                {
                    std::shared_ptr<Component> l_infant = std::allocate_shared<Component, std::pmr::polymorphic_allocator<Component>>(&m_component_arena);
                    archetype_it->second.emplace(std::move(l_identifier), l_infant);
                    return l_infant;
                }

                FE_LOG("Warning, request_component_attachment() failed: the component already exists within the archetype named '${%s@0}.'", reflection::type_id<Archetype>().name());
                return ECS::nullcom<Component>;
			}
        }
    ...
    void request_component_detachment(Archetype* const entity_p) noexcept
    {
        ...
        for (auto archetype_it = m_component_table.find(entity_p); archetype_it != m_component_table.end(); ++archetype_it)
        {
            if (archetype_it->first == entity_p)
            {
                ...
                if (l_component_it != archetype_it->second.end())
                {
					archetype_it->second.erase(l_identifier);
                    return;
                }

                FE_LOG("Warning, request_component_detachment() failed: the component does not exist within the archetype named '${%s@0}.'", reflection::type_id<Archetype>().name());
                return;
            }
        }
    ...
    component_ptr<Component> request_component_retrieval(Archetype* const entity_p) noexcept
    {
        ...
        for (auto archetype_it = m_component_table.find(entity_p); archetype_it != m_component_table.end(); ++archetype_it)
        {
            if (archetype_it->first == entity_p)
            {
                ...
                if (l_component_it != archetype_it->second.end())
                {
                    return l_component_it->second;
                }

                FE_LOG("Warning, request_component_retrieval() failed: the component does not exist within the archetype named '${%s@0}.'", reflection::type_id<Archetype>().name());
                return ECS::nullcom<Component>;
            }
        }
	...
	void register_system() noexcept
	{
        ...
        std::pmr::string l_identifier(reflection::type_id<System>().name(), FE::framework::framework_base::get_engine().get_memory_resource());
        if (this->m_entity_table.find(l_identifier)->first == l_identifier)
        {
