...
TEST(allocator, new_delete_allocator)
{
	...
	{
		int* l_ptr = FE::new_delete_allocator<int>::allocate(50);
		EXPECT_EQ(FE::heap_utilization<int>::query_type_data()._thread_local_total_bytes_by_type, 200);

		l_ptr = FE::new_delete_allocator<int>::reallocate(l_ptr, 50, 100);
		EXPECT_EQ(FE::heap_utilization<int>::query_type_data()._thread_local_total_bytes_by_type, 400);

		l_ptr = FE::new_delete_allocator<int>::reallocate(l_ptr, 100, 10);
		EXPECT_EQ(FE::heap_utilization<int>::query_type_data()._thread_local_total_bytes_by_type, 40);

		FE::new_delete_allocator<int>::deallocate(l_ptr, 10);
	}
	EXPECT_EQ(FE::heap_utilization<int>::query_all_data()._global_total_bytes, 0);
}
