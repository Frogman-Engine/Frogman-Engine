...
TEST(fstring, insert)
{
	FE::fstring<_TEST_FSTRING_LENGTH_> l_fstring = "etreme programming is evil.";
	l_fstring.insert(1, 1, 'x');
	EXPECT_TRUE("extreme programming is evil." == l_fstring);
...
TEST(fstring, copy)
{
	...
	l_fstring.copy(l_buffer, FE::capacity_t{ 20 }, count_t{ 5 }, index_t{ 24 });
...
TEST(fstring, rfind)
{
	...
	std::optional<string::range> l_search_result = l_fstring.rfind(l_substring, 39);
	EXPECT_TRUE(l_search_result.has_value());

	EXPECT_TRUE(string::compare_ranged_strings(l_fstring.data(), *l_search_result, l_substring.data(), string::range{0, l_substring.length()}));


	std::optional<string::char_info<char>> l_result2 = l_fstring.rfind('t', index_t{ 20 });
